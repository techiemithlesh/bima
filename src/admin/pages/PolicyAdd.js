...
import { validateBusinessType, validateCustomerName, validateFuelType, validateMobileNumber, validateNetAmount, validateOwnDamage, validatePaymentMethod, validatePolicyNumber, validatePolicyType, validateRegistrationNo, validateRiskStartDate, validateThirdParty, validateVehicleManufacture, validateVehicleModel, validateVehicleRegistrationDate, validateVehicleType } from "../../utils/Validation";
...
const PolicyAdd = () => {
    ...
    function generateBreadcrumbData(selectedTabIndex) {
        let middleContent;
        let isSaveDisabled = true;
    ...
    const requiredFieldsByTab = [
        ['business_type', 'policy_type', 'vehicle_type', 'fuel_type', 'registration_number', 'vehicle_manufacture', 
        'vehicle_model'],
        ['insurer', 'net_amount', 'payment_type', 'policy_number', 'risk_start_date', 'own_damage', 'third_party'],
    ...
    return (
        <Layout title="Add Policy" className="policy" breadcrumbData={generateBreadcrumbData(selectedTabIndex)}>
            <Card>
                <Tabs selectedIndex={selectedTabIndex} onSelect={handleTabSelect}>
                    <TabList style={{ display: "flex", margin: 0, padding: 0 }} className="policytab">
                        {[1, 2, 3, 4, 5].map((_, index) => (
                            <Tab key={index} style={{ flex: 1, textAlign: "center", padding: "10px" }}>
                    ...
                    {selectedTabIndex === 4 && (
                        <div className="flex justify-center">
                            <button onClick={handleSave} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-8 save">
                                Save
                            </button>
                        </div>
                    )}
